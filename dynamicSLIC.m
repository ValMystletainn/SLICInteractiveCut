% dynamic SLIC for show in a now figure, it will show result by the end of
% each iteration
function dynamicSLIC(I, k, m, IOriginal) % S = sqrt(N / k)
...
%         imagesc(L)
    ...
    % put center
    hold on
    for centerCount = 1 : size(centerFeatures, 1)
        centerM = centerFeatures(centerCount, 1);
        centerN = centerFeatures(centerCount, 2);
        plot(centerN, centerM,'ro','MarkerFaceColor','r');
    end
    pause(0.1)
    if (isStable)
        break
